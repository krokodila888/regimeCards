...
import type { WorkflowState, RegimeArrow } from '../types/chart-data';
...
export default function Stage3RegimeEditing({
  ...
  const getSidebarArrowColor = (canvasColor: string): string => {
  ...
  const handleCalculateActualCurve = async () => {
    if (!workflow.regimeArrows || workflow.regimeArrows.length === 0) return;
    ...
    for (let km = 0; km <= trackLength; km += 0.5) {
      ...
      const arrow = workflow.regimeArrows.find((a) => km >= a.startKm && km <= a.endKm);
      ...
      const mode = workflow.locomotive?.tractionModes.find((m) => m.id === arrow.modeId);

      // Get speed limit at this position
      const speedLimits = workflow.trackSection?.speedLimits || [];
      const limit = speedLimits.find((sl) => km >= sl.startCoord && km <= sl.endCoord);
      const maxSpeed = limit ? limit.limitValue : 120;

      // Calculate relative position within arrow (0 to 1)
      const relativePos = (km - arrow.startKm) / (arrow.endKm - arrow.startKm);
      ...
      if (mode?.label.includes('100%') || mode?.label.includes('MAX')) {
        targetSpeed = maxSpeed * 0.95;
      } else if (mode?.label.includes('85%')) {
        targetSpeed = maxSpeed * 0.93;
      } else if (mode?.label.includes('80%') || mode?.label.includes('70%')) {
        targetSpeed = maxSpeed * 0.92;
      } else if (mode?.label.includes('50%') || mode?.label.includes('60%')) {
        targetSpeed = maxSpeed * 0.88;
      } else if (mode?.label.includes('30%') || mode?.label.includes('40%')) {
      ...
      } else if (mode?.label.includes('СП-4ОП') || mode?.label.includes('2СП-4ОП')) {
      ...
      if (relativePos < transitionLength) {
        ...
        const prevSpeed = km > 0 ? rawCurve[rawCurve.length - 1]?.speed || 0 : 0;
        speed =
          prevSpeed + ((targetSpeed - prevSpeed) * (1 - Math.cos(accelProgress * Math.PI))) / 2;
      } else if (relativePos > 1 - transitionLength) {
        // Check if there's a next arrow
        const nextArrow = workflow.regimeArrows.find((a) => a.startKm === arrow.endKm);

        if (nextArrow) {
          // Smooth transition to next arrow's speed
          const decelProgress = (relativePos - (1 - transitionLength)) / transitionLength;

          // Estimate next arrow's target speed
          const nextMode = workflow.locomotive?.tractionModes.find(
            (m) => m.id === nextArrow.modeId
          );
          let nextTargetSpeed = maxSpeed * 0.9;
          if (nextMode?.label.includes('100%') || nextMode?.label.includes('MAX')) {
            nextTargetSpeed = maxSpeed * 0.95;
          } else if (nextMode?.label.includes('80%') || nextMode?.label.includes('70%')) {
            nextTargetSpeed = maxSpeed * 0.92;
          } else if (nextMode?.label.includes('50%') || nextMode?.label.includes('60%')) {
            nextTargetSpeed = maxSpeed * 0.88;
          } else if (nextMode?.label.includes('30%') || nextMode?.label.includes('40%')) {
            nextTargetSpeed = maxSpeed * 0.82;
          }

          speed =
            targetSpeed +
            ((nextTargetSpeed - targetSpeed) * (1 - Math.cos(decelProgress * Math.PI))) / 2;
        }
      }
    ...
    if (smoothedCurve.length > 0) {
      ...
      for (let i = 1; i < Math.min(4, smoothedCurve.length); i++) {
        const progress = i / 4;
        smoothedCurve[i].speed = (targetStartSpeed * (1 - Math.cos(progress * Math.PI))) / 2;
      }
      ...
      let targetEndSpeed = smoothedCurve[lastIndex - 4]?.speed || 60;
      ...
      for (let i = 1; i < Math.min(4, smoothedCurve.length); i++) {
        ...
        if (index > 0) {
          smoothedCurve[index].speed =
            (targetEndSpeed * (1 - Math.cos((1 - progress) * Math.PI))) / 2;
        }
      }
    }
  ...
  if (isLocked) {
    console.log('[Stage3RegimeEditing] Component is locked. isLocked:', isLocked);
    return <div className="text-center text-gray-500 text-sm py-8">Завершите предыдущие этапы</div>;
  }

  if (!workflow.locomotive || !workflow.optimalSpeedCurve) {
    console.log('[Stage3RegimeEditing] Missing required data:', {
      hasLocomotive: !!workflow.locomotive,
      locomotiveId: workflow.locomotive?.id,
      locomotiveName: workflow.locomotive?.name,
      hasOptimalSpeedCurve: !!workflow.optimalSpeedCurve,
      optimalSpeedCurveLength: workflow.optimalSpeedCurve?.length,
      workflowKeys: Object.keys(workflow),
      trackSection: workflow.trackSection?.name,
      trackSectionId: workflow.trackSection?.id,
    });
    return (
  ...
  console.log('[Stage3RegimeEditing] Component ready. Workflow state:', {
    hasLocomotive: !!workflow.locomotive,
    locomotiveName: workflow.locomotive?.name,
    tractionModesCount: workflow.locomotive?.tractionModes?.length,
    hasOptimalSpeedCurve: !!workflow.optimalSpeedCurve,
    optimalSpeedCurvePoints: workflow.optimalSpeedCurve?.length,
    hasRegimeArrows: !!workflow.regimeArrows,
    regimeArrowsCount: workflow.regimeArrows?.length,
    hasActualSpeedCurve: !!workflow.actualSpeedCurve,
    actualSpeedCurvePoints: workflow.actualSpeedCurve?.length,
  });
  ...
  return (
    <div className="space-y-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: getSidebarArrowColor(mode.color),
                    }}
                  />
                </div>
              </button>
      ...
      {workflow.regimeArrows && workflow.regimeArrows.length > 0 && (
        <Button
